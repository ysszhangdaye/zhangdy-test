package com.zhangdy.test.chat.processor;
...
import com.zhangdy.test.chat.protocol.IMP;
...
public class MessageProcessor {
    ...
    public void logout(Channel client){
        ...
        for (Channel channel : onlineUsers) {
            IMMessage request = new IMMessage(IMP.SYSTEM.getCommand(), sysTime(), onlineUsers.size(), getNickName(client) + "离开");
    ...
    public void sendMsg(Channel client,String msg){
        ...
        if(request.getCmd().equals(IMP.LOGIN.getCommand())){
...
//			System.out.println(client.attr(FROM).get());
            ...
            for (Channel channel : onlineUsers) {
                ...
                if(!isself){
                    request = new IMMessage(IMP.SYSTEM.getCommand(), sysTime(), onlineUsers.size(), getNickName(client) + "加入");
                }else{
                    request = new IMMessage(IMP.SYSTEM.getCommand(), sysTime(), onlineUsers.size(), "已与服务器建立连接！");
                }
        ...
        }else if(request.getCmd().equals(IMP.CHAT.getCommand())){
            for (Channel channel : onlineUsers) {
                boolean isself = (channel == client);
                if (isself) {
        ...
        }else if(request.getCmd().equals(IMP.FLOWER.getCommand())){
            ...
            if(null != attrs){
                ...
                if(sub < 1000 * secends){
                    request.setSender("you");
                    request.setCmd(IMP.SYSTEM.getCommand());
